...
if st.button("Display Bet"):
    ...
    username, bet_selection, wager, earned_payout, bet_status = contract.functions.reviewBet(betID).call()
...
if st.button("updateBet"):
    try:
        contract.functions.updateBet(betID, new_earned_payout, new_bet_status).transact({'from': user_account, 'gas': 1000000})
